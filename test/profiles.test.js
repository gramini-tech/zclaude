...
describe("profiles", () => {
  ...
  before(async () => {
    ...
    await writeFile(join(dir, "work.env"), "# name: Work MCP\n# description: adds MCP env\nMY_MCP_TOKEN=abc\nZCLAUDE_ZAI=1\n");
  ...
  it("lists built-ins first, then valid user profiles", async () => {
    ...
    assert.deepEqual(profiles.slice(0, 2).map((p) => p.id), BUILTIN_PROFILES.map((p) => p.id));
    assert.deepEqual(profiles.slice(2).map((p) => p.id), ["plain", "work"]);
