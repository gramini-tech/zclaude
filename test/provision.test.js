...
function bizFetch({ existingKeys = [], customer, createResponse } = {}) {
  return mockFetch(({ url, method }) => {
    if (url === config.bizLoginUrl && method === "POST") return bizEnvelope({ access_token: "biz-token", expires_in: 3600 });
    if (url === `${config.apiBase}/api/biz/customer/getCustomerInfo`) {
      return bizEnvelope(customer ?? { organizations: [{ organizationId: "org-1", isDefault: true, projects: [{ projectId: "proj-1", isDefault: true }] }] });
    }
    if (url === KEYS_URL && method === "GET") return bizEnvelope(existingKeys);
    if (url === KEYS_URL && method === "POST") return createResponse ?? bizEnvelope({ name: "zclaude", apiKey: "created-id", secretKey: "inline-ignored" });
    if (url.startsWith(`${KEYS_URL}/copy/`)) return bizEnvelope({ apiKey: decodeURIComponent(url.slice(`${KEYS_URL}/copy/`.length)), secretKey: "real-secret" });
    return undefined;
  });
}
...
describe("pickOrgProject", () => {
  it("prefers isDefault, then the Chinese default name, then the first entry", () => {
    assert.deepEqual(pickOrgProject({ organizations: [
    ...
    assert.deepEqual(pickOrgProject({ organizations: [
      ...
      { organizationId: "b", organizationName: "默认机构", projects: [{ projectId: "y", projectName: "other" }, { projectId: "z", projectName: "默认项目" }] },
    ] }), { organizationId: "b", projectId: "z" });
    assert.deepEqual(pickOrgProject({ organizations: [{ organizationId: "only", projects: [{ projectId: "one" }] }] }), { organizationId: "only", projectId: "one" });
  });
...
describe("mintApiKey", () => {
  it("creates a key when none matches and copies the secret", async () => {
    ...
    assert.deepEqual(fetchImpl.calls.map((call) => `${call.method} ${call.url}`), [
  ...
  it("reuses an existing key by name", async () => {
    const fetchImpl = bizFetch({ existingKeys: [{ name: "other", apiKey: "nope" }, { name: "zclaude", apiKey: "existing-id" }] });
    const result = await mintApiKey("oauth-short", config, { fetchImpl });
    assert.equal(result.apiKey, "existing-id.real-secret");
    assert.equal(result.created, false);
    assert.ok(!fetchImpl.calls.some((call) => call.method === "POST" && call.url === KEYS_URL));
  });
  ...
  it("surfaces envelope failures and adds the key-limit hint on create", async () => {
    const fetchImpl = bizFetch({ createResponse: jsonResponse({ code: 1300, msg: "key limit reached", success: false }) });
    await assert.rejects(mintApiKey("t", config, { fetchImpl }), (error) => {
  ...
  it("refuses an empty token", async () => {
    await assert.rejects(mintApiKey("", config, { fetchImpl: mockFetch(() => undefined) }), /without an OAuth access token/u);
  });
});
