...
export function knownModels() {
  return Object.keys(MODEL_CONTEXT_WINDOWS).map((id) => ({ id, contextWindow: MODEL_CONTEXT_WINDOWS[id] }));
}
...
async function pickModel({ message, models, current }) {
  const value = await guard(select({
    message,
  ...
  return (await guard(input({
    message: "Model id:",
    default: current,
    validate: (text) => (/^[A-Za-z0-9][A-Za-z0-9._\-[\]]*$/u.test(String(text).trim()) ? true : "Use letters, digits, dots and dashes only."),
  }))).trim();
}
...
export async function runModelWizard({ models, current }) {
  const list = models.length ? models : knownModels();
  const primary = await pickModel({ message: "Primary model (what you talk to):", models: list, current: current.primary });
  const subagent = await pickModel({
    message: "Subagent model (background agents, Task tool):",
    models: list,
    current: list.some((m) => m.id === current.subagent) || !list.length ? current.subagent : primary,
  });
...
export async function chooseSaveLocation({ projectPath, userPath, userExists }) {
  return guard(select({
...
export async function promptApiKey() {
  const value = await guard(password({
    message: "Paste your Z.ai coding-plan API key:",
    mask: "*",
    validate: (text) => (String(text).trim().length >= 16 ? true : "That does not look like a Z.ai key (expected id.secret)."),
  }));
  return value.trim();
}

export async function confirmChoice(message, choices, defaultValue) {
